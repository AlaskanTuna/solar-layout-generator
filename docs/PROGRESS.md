...
- Updated System Cost tooltip with transparent breakdown: panel cost × installation multiplier, with plain-English explanation of what the multiplier covers (inverter, mounting, wiring, labour, permitting).
- Expanded Financial Disclaimers: added system cost methodology disclaimer and exclusions notice (maintenance, inverter replacement, tariff escalation, inflation).
- Added Financial Roadmap card (Advanced view) after Net Benefit Projection: dynamic milestones showing initial investment, break-even year, inverter replacement window (year 10–15), and warranty-end output projection.
- Clarified `costPerWp` comments in `shared/index.ts` and `shared/panelModels.ts` to indicate it is panel module cost only.
- Verification: `pnpm build` clean, all 71 tests passing (47 frontend + 24 backend).

## [30/03/26] - Tooling Migration: npm -> pnpm

- Migrated the monorepo from npm workspaces to pnpm workspaces by adding `packageManager`, `pnpm-workspace.yaml`, and a committed `pnpm-lock.yaml`, while updating the root/workspace scripts to use pnpm filters.
- Updated Heroku-facing runtime config so `Procfile` starts the app with `pnpm start`, `heroku-postbuild` stays on the root pnpm build, and the old `package-lock.json` is removed to prevent npm fallback during deploys.
- Updated GitHub Actions CI/CD to install with pnpm, cache the pnpm lockfile, and keep the Heroku deploy flow aligned with the new package manager.
- Verification: `pnpm install`, `pnpm build`, and `pnpm test` all pass on Node `v24.14.0`.

## [27/03/26] - Phase 6: UI/UX Production Revamp (Subphases 6.0–6.7)

- **6.0 Design Foundation:** Solar orange color palette with full light/dark mode (CSS variables in `:root` and `.dark`). Google Fonts (Outfit headings + Work Sans body). Glassmorphism utility classes (`.glass`, `.glass-card`, `.glass-nav`). Custom CSS animations (fade-in, fade-in-up, scale-in, glow-pulse). `ThemeProvider` with localStorage persistence and system theme detection. `ThemeToggle` component. `prefers-reduced-motion` respected globally.
- **6.1 Landing Page:** Complete redesign — hero with gradient orbs and animated glow, "How It Works" 3-step flow, 4-feature showcase with glass cards, testimonial section (3 UAT participants), pricing preview (Free/Pro/Enterprise), trust signals, disclaimer section, full footer with links.
- **6.2 Auth Pages:** Split-screen layout — left panel with orange gradient + branding, right panel with centered form. Animated loading states, theme toggle. Email confirmation success state with icon.
- **6.3 App Shell:** `AppNav` component — glass navbar with logo, breadcrumbs (Dashboard > Map > Workbench > Analysis), user dropdown (email, plan badge, sign out), theme toggle. `dropdown-menu` shadcn component added.
- **6.4 Dashboard:** Glass cards for project list, quick stats bar (total/completed/in-progress), empty state with onboarding illustration, status badges with icons per status.
- **6.5 MapPage:** Replaced hardcoded `bg-white`/`text-stone-*` with semantic tokens (`glass`, `bg-card`, `text-foreground`). Glass cards for confirm/processing/error panels.
- **6.6 WorkbenchPage:** Replaced all hardcoded stone colors in WorkbenchSidebar, CanvasControls, PanelModelDrawer with semantic tokens. Loading overlays use glass effect. Canvas container border uses `border-border`.
- **6.7 AnalysisPage:** Replaced all hardcoded stone colors in sidebar controls, analysis sub-components (BillBreakdown, HeroMetrics, MonthTable, charts) with semantic tokens.
- **Cross-cutting:** GuidedTour, FloatingNav, LoadingOverlay, AppErrorBoundary, ImagePopup all updated to semantic tokens. Sonner toaster now theme-aware. ProtectedRoute spinner uses primary color. 404 page with branded design.
- **Verification:** TSC clean (shared + frontend), all 47 tests passing, Prettier compliant.

## [27/03/26] - Frontend Code Refactoring (Pre-Phase 6)

- **Tier 2 — Global Config:** Created `frontend/src/lib/constants.ts` with centralized `COLORS` and `CHART_TOOLTIP_STYLE` tokens. Added `colors.solar.*` semantic tokens to `tailwind.config.js`. Extracted shared `FloatingNav` component from duplicated nav pattern across WorkbenchPage and AnalysisPage.
- **Tier 1 — AnalysisPage (1636→912 lines):** Extracted 8 sub-components into `components/analysis/`: `formatters.ts` (shared formatters + tooltip constants), `HeroMetrics`, `BillComparisonChart`, `NetBenefitChart`, `BillBreakdown`, `MonthTable`, `SimplePdfReport`, `AdvancedPdfReport`. Page retains state orchestration and sidebar inline.
- **Tier 1 — WorkbenchPage (1806→1307 lines):** Extracted `WorkbenchSidebar` and `CanvasControls` into `components/workbench/`. Extracted `useWorkbenchKeyboard` and `useIrradiance` into `hooks/`. Page retains Konva canvas, state management, and handler logic.
- **Tier 3 — Backend Cleanup:** No stale build artifacts found (already cleaned in prior session).
- **Verification:** TSC clean (shared + frontend), all 47 tests passing, Prettier compliant.

## [27/03/26] - QA Fixes + Simple PDF Export

- **MapPage:** Nav buttons repositioned to left/right screen edges (Dashboard left, Workbench right) using `justify-between`. Search box restored to `top-4` since nav no longer overlaps.
- **WorkbenchPage — Irradiance Glow:** Replaced broken absolute overlay div with CSS `box-shadow: inset` on the canvas container. Gradient rotates by month azimuth, larger and softer amber glow. Exported `MONTHLY_AZIMUTH` from IrradianceGlow.tsx. Canvas shrunk to `min-h-[50vh]`. Month slider label changed from confusing "X% solar" to descriptive sun emoji + month name.
- **WorkbenchPage — Spacebar Scroll:** Fixed by making root div `h-screen overflow-hidden` and inner layout `overflow-y-auto`. Also ensured `e.preventDefault()` on spacebar.
- **WorkbenchPage — Snap Alignment:** Rewrote `computeSnap` to use oriented-edge projection. Projects center-to-center delta onto panel's local rotation axes (U along width, V along height), snaps to exact panel-width/height distances (edge-to-edge). Properly handles rotated panels.
- **WorkbenchPage — Segments Overlay:** Fixed convex hull for small point sets (2-point fallback to padded rectangle). Increased hull padding (20px margin around points). Works for segments with few panels.
- **WorkbenchPage — Mask Legend:** Left-justified mask overlay legend text.
- **WorkbenchPage — Panel Model Drawer:** Moved from sidebar to bottom-center floating nudge/pill trigger. Drawer trigger is a compact rounded-full pill showing model name + capacity with pull-up chevron.
- **WorkbenchPage — 3D Panel Previews:** Added Canvas-generated cell grid textures (6x10 grid lines + bus bars), glass reflection layer, aluminum frame. Each model gets a unique cell color via `MODEL_CELL_COLORS` map.
- **WorkbenchPage — Animations:** Added `active:scale-90` to all canvas control buttons. Smooth overlay expand/collapse via `max-h` + `opacity` CSS transition (always in DOM, animated height).
- **WorkbenchPage + AnalysisPage — Navigation:** Replaced sidebar nav links with floating top-of-page buttons (MapPage style). WorkbenchPage: absolute positioned Dashboard/Map buttons. AnalysisPage: sticky positioned Workbench/Dashboard buttons.
- **Global Haptic Animations:** Added `active:scale-[0.98]` to Button component base styles for subtle press feedback.
- **Simple PDF Export (Phase 5.7):** Added `simpleReportRef` with 1-page homeowner summary layout (hero metrics, plain-language paragraph, monthly bill comparison table). When `viewMode === 'simple'`, Export PDF uses the simple layout instead of the full report. Trimmed Phase 5.7 in PLAN.md to only this task.
- **Verification:** TSC clean across shared/backend/frontend. 47 frontend tests pass.

## [26/03/26] - Phases 5.2.4–5.2.8: Workbench Feature Expansion

- **Feedback Fixes:** Repositioned MapPage nav buttons to center (Dashboard left, Workbench right with forward arrow). Removed "Step 1 of 3" label. Fixed group drag bug (onDragStart was clearing multi-selection). Fixed marquee coordinate offset by reading transform from Konva stage node instead of React state closures. Added spacebar+drag canvas pan mode (prevents deselecting marquee selection). Centered all canvas control tooltips using `right-full mr-2`. Removed selected count indicator. Updated tour steps for new controls. Cleaned 147 TSC build artifacts from frontend/src, added `noEmit: true` to frontend tsconfig.
- **Mask Overlay (5.6.6):** Extended backend `GET /api/locations/:locationId/overlay/mask` to colorize `mask.tif` as green-tinted PNG (valid roof area). Added Mask button to frontend overlay dropdown with green gradient swatch.
- **Roof Segment Visualization (5.6.4):** Created `segmentVisualization.ts` with convex hull (Graham scan) and azimuth-based HSL color mapping. Groups visible panels by `segmentIndex`, renders semi-transparent Konva.Line hulls per segment. Added Segments toggle to overlay dropdown (independent of image overlay). Info panel in bottom-left shows per-segment azimuth, pitch, and panel count.
- **Canvas Controls Expansion (5.6.5):** Reorganized controls into labeled groups (Tools, View, Layers) with consistent dividers and uppercase micro-labels. All controls use `right-full mr-2` tooltip positioning.
- **Panel Model Selector Drawer (5.6.7):** Replaced sidebar `<select>` with `vaul` bottom-sheet drawer triggered by a clickable model card. Horizontal scrollable card layout with model specs (Wp, efficiency, dimensions, cost/Wp). Each card has a lazy-loaded Three.js spinning 3D panel preview (`@react-three/fiber` + `@react-three/drei`). Selected model highlighted with blue border and checkmark. Installed `vaul`, `three`, `@react-three/fiber`, `@react-three/drei`.
- **Irradiance Ambient Indicator (5.6.8):** Created `IrradianceGlow` component with directional radial CSS gradient (amber #FFB800). Sun azimuth approximated per month for Malaysia (~3°N). Gradient positioned on canvas edge by azimuth, opacity scaled by monthly irradiance factor. Month slider (0–11) below canvas reactively drives glow direction and intensity.
- **Verification:** Frontend + backend TSC clean. 47 frontend tests pass.

## [26/03/26] - Phases 5.1–5.2.3: MapPage Polish + WorkbenchPage Interaction Enhancements

- **MapPage Polish (5.5):** Added top-left navigation buttons (Dashboard always visible, Workbench in readonly mode). Added "Step 1 of 3" label for new projects. Added readonly CTA linking to Dashboard for different-location analysis. Set `disableDefaultUI: true` on Google Maps to strip streetView/fullscreen clutter (kept zoom + satellite toggle). Moved guided tour `?` button from bottom-right to bottom-left to avoid collision with Maps zoom controls. Consolidated duplicate `Link` import.
- **Undo/Redo (5.6.1):** Created generic `useUndoRedo<T>` hook with bounded history (max 30 snapshots), ref-based storage with render trigger. Integrated into `usePanelState` — snapshots captured before every move, rotate, delete, and visibility change. `undo()`/`redo()` restore full `{panels, visibleCount}` state. Wired Ctrl+Z (undo) and Ctrl+Shift+Z/Ctrl+Y (redo) keyboard shortcuts. Added undo/redo icon buttons to canvas controls toolbar with disabled state at boundaries.
- **Multi-Select + Group Operations + Marquee (5.6.2):** Replaced `selectedPanelId: string | null` with `selectedPanelIds: Set<string>`. Shift+click toggles panel in/out of selection; regular click selects one. Multi-selected panels show cyan stroke (vs white for single). Group drag applies pixel delta to all selected, validates all (with `excludeIds` for inter-group overlap), batch recomputes on success, rollback all on failure. Group rotate applies same absolute rotation with debounced batch recompute. Group delete via Delete/Backspace key. Click empty canvas deselects all. Rectangular marquee tool: toggle button in canvas controls, click+drag on Stage draws dashed cyan Konva.Rect, mouseup selects panels whose centers fall inside. Stage drag disabled when marquee active. Sidebar shows multi-selection info with count and instructions.
- **Snap Alignment (5.6.3):** Created `snapAlignment.ts` utility — compares dragged panel AABB edges against all visible panels with similar rotation (±5°), snaps X and Y independently within 8px threshold. Integrated via `dragBoundFunc` callback chain: PanelRect accepts `snapEnabled` + `onSnapDragMove` props, PanelLayer passes through, WorkbenchPage provides `handleSnapDragMove` callback using `computeSnap`. Renders temporary cyan dashed `Konva.Line` guide lines during snapping. Added magnet icon toggle to canvas controls (default ON). Guides clear on drag end.
