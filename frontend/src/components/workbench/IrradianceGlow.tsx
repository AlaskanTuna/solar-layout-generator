...
export function IrradianceGlow({ month }: IrradianceGlowProps) {
  const style = useMemo(() => {
    ...
    // Convert azimuth to CSS gradient position
    // Satellite images are north-up, so:
    // azimuth 0° (N) → gradient from top
    // azimuth 90° (E) → gradient from right
    // azimuth 180° (S) → gradient from bottom
    // azimuth 270° (W) → gradient from left
