...
Treat every file under `docs/` as local-only workflow material. Keep those files ignored by git and never add them to commits unless the project owner explicitly asks for a tracked documentation change.
...
### 4. UAT.md (User Acceptance Testing Log)

`docs/UAT.md` is a living document that tracks **timestamped feedback from targeted users** (e.g. Malaysian homeowners) throughout the RAD development cycles. As the developer conducts UAT sessions, feedback entries are appended here. Agents should **read this document before working on UI/UX improvements** to incorporate real user insights. After addressing feedback, agents should update the relevant entry's status.
...
| --- | ------------- | --------------------------------------- | -------- | ------------------- |
| 1   | WorkbenchPage | "Dragging panels feels laggy on mobile" | High     | Open                |
| 2   | AnalysisPage  | "Monthly bill chart is hard to read"    | Medium   | Resolved (17/03/26) |

**General Comments:** Free-form participant remarks.
```

- **Severity:** Critical / High / Medium / Low
- **Status:** Open / In Progress / Resolved (DD/MM/YY)
